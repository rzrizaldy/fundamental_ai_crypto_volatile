...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--base-url", default="http://localhost:8010", help="FastAPI base URL.")
...
def main() -> None:
    ...
    try:
        health = _http_json(f"{args.base_url}/health")
        version = _http_json(f"{args.base_url}/version")
    except error.URLError as exc:
        raise SystemExit(
            f"Could not reach the Week 4 API at {args.base_url}. Start it with `python scripts/run_w4_api.py`. ({exc})"
        ) from exc
    ...
    for start in range(0, replay_rows, batch_size):
        payload = _http_json(
            ...
            method="POST",
            payload={"replay_count": batch_size, "replay_start_index": start},
        )
        batches += 1
        total_scored += int(payload["rows_scored"])
        for row in payload["predictions"]:
            last_probability = row["probability"]
            predicted_positive += int(row["predicted_label"])
